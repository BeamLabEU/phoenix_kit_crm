defmodule PhoenixKitCRM.SchemaOwnerGuardTest do
  @moduledoc """
  ...
  @scratch_db "i067_schema_owner_guard_scratch"
  ...
  test "a different package's marker: check! raises the legible refusal message", %{
    ...
    query_fn.("CREATE TABLE schema_migrations (version bigint)")
    query_fn.("COMMENT ON TABLE schema_migrations IS 'phoenix_kit_document_creator'")
    ...
    assert error.message =~
             "PGDATABASE points at a database whose migration history belongs to " <>
               "phoenix_kit_document_creator, not phoenix_kit_crm"
