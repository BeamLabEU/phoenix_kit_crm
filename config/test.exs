...
# Or point PGDATABASE at an already-provisioned shared instance (e.g. the
# workspace's `migration_test_db`, see /www/.tools/test-db.env) — same
# override `phoenix_kit_document_creator`'s config/test.exs uses.
...
config :phoenix_kit_crm, PhoenixKitCRM.Test.Repo,
  ...
  database:
    System.get_env(
      "PGDATABASE",
      "phoenix_kit_crm_test#{System.get_env("MIX_TEST_PARTITION")}"
    ),
