defmodule PhoenixKitCRM.Web.CompanyFormLive do
  ...
  def handle_event("mirror_resolve", %{"choices" => raw_choices}, socket) do
    company = socket.assigns.company
    user_uuid = socket.assigns.mirror_pending_user_uuid

    with %User{} = user <- Auth.get_user(user_uuid),
         choices = atomize_choices(raw_choices),
         deltas = Mirror.resolve(:company, company, user, choices),
         {:ok, {linked_company, linked_user}} <-
           Companies.apply_mirror_resolution(company, user, deltas) do
      {:noreply,
       socket
  ...
  # Whitelisted conversion — never String.to_existing_atom/1 a submitted
  # param directly. Unknown field names / choice values are silently
  # dropped rather than raising on a forged payload.
  defp atomize_choices(raw_choices) when is_map(raw_choices) do
  ...
  def render(assigns) do
    ...
    <.mirror_conflict_modal conflicts={@mirror_conflicts} master={:crm} show={@show_conflict} />
    """
  end
